...
# If the first column is numeric, we will convert it to character with zeros at the beginning to make it 5 characters long
data$`Code Commune` <- sprintf("%05d", data$`Code Commune`)
head(data)
```  
